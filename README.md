...
수면 관련 질문에 문헌 근거를 연결하는 RESTDAWN RAG 모듈의 포트폴리오 사례 연구입니다. 의미 검색과 키워드 검색을 결합하도록 설계했으며, 제출 소스의 연결 오류와 수정 범위를 함께 기록합니다.

**개인 기여: RAG 모델 구축·Qdrant/Lucene 하이브리드 검색·재랭킹.** 전체 백엔드는 팀 프로젝트이며, 이 저장소는 공개 권한이 확인되지 않은 팀 소스 대신 구현 분석과 개인 기여를 정리합니다.

## Project Overview

일반적인 수면 조언에 외부 문헌 근거를 연결하는 것이 목표입니다. 제출 소스에서 문서 색인, 질의 확장, RRF 병합, 다양성 재랭킹, 인용 문맥 구성, LLM 호출 구조를 확인했습니다. 2026-09-09 재검토에서는 Dense 어댑터가 검색 메서드가 없는 임베딩 클라이언트에 연결되어 빈 결과를 반환하는 오류를 확인했습니다. 아래 구조도는 설계와 호출 구조를 나타내며, 제출본의 의미 검색이 정상 동작했다는 증명은 아닙니다. 실제 서비스의 치료 효과도 검증하지 않았습니다.
...
**검증 상태:** 공개 저장소에는 실행 소스가 없습니다. 개인 보관 사본에서 Dense 연결·레거시 청크 색인·민감한 질의 로그·Dockerfile의 wrapper 복사를 수정했습니다. 청크 메서드 회귀 검사와 Spring AI 1.0.3 API 기반 Dense 계약 검사는 통과했으나, 팀 앱 전체 빌드 및 Qdrant/OpenAI 통합 실행은 미검증입니다. Docker 엔진 연결도 불가능해 이미지 빌드는 실행하지 못했습니다. [수정 내역과 검증 범위](docs/review-followup.md)를 참고하세요.
...
위 항목은 코드에 있는 처리 단계입니다. Dense 경로 연결 오류와 파일 색인의 저장소 불일치 때문에 전체 파이프라인의 정상 동작은 별도로 검증해야 합니다.

## Data Boundaries

사용자 질문과 선택한 프로필은 서버에서 프롬프트로 구성되고 OpenAI 모델에 전달됩니다. 전체 서비스의 상담 경로에는 서버 DB에 사용자·AI 메시지를 저장하는 코드도 있습니다. 따라서 기기에서 모든 처리가 끝난다거나 개인정보 유출 위험이 없다고 설명할 수 없습니다. API 인증과 관리자 권한 검사는 존재하지만 외부 전송·보관·삭제 정책까지 검증한 상태는 아닙니다. [데이터 흐름과 설명 기준](docs/review-followup.md#데이터-흐름과-개인정보)을 참고하세요.
